...
char* rgb = "{\"deviceType\":\"LightingControl\",\"deviceCode\": \"rgb5\",\"numberOfPorts\": 1, \"lights\": [{\"portId\": 1, \"dimmable\": false, \"typeOfLight\": \"RGB\"}]}";
String rgb_id = "";

char* pir = "{\"deviceType\":\"SensorModule\",\"deviceCode\": \"pir5\",\"sensors\": [{\"name\": \"PIR1\", \"_type\": \"Pir\", \"value\": 0}]}";
String pir_id = "";

char* doors = "{\"deviceType\":\"SensorModule\",\"deviceCode\": \"doors5\",\"sensors\": [{\"name\": \"Doors1\", \"_type\": \"Door\", \"value\": 0}]}";
String doors_id = "";
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  } else  if ((String)topic == "authenticate/rgb5") {
  ...
  } else if ((String)topic == "authenticate/pir5") {
    timer.stop(authenticate_timer);
    pir_id = rmsg;
  ...
  } else if ((String)topic == "authenticate/doors5") {
    timer.stop(authenticate_timer);
    doors_id = rmsg;
...
void checkMqttConnection() {
  ...
  if (!client.connected()) {
    ...
    // Attempt to connect
    if (client.connect("rgb5", "lwt", 0, false, "rgb5")) {
      Serial.println("connected");
      //client.publish("connected", "hello");\
      delay(10);
      client.subscribe("authenticate/rgb5");
      client.subscribe("authenticate/pir5");
      client.subscribe("authenticate/doors5");
...
void readPirSensorValue() {
  ...
  if (eValue != eValue_previous) {
    ...
    sprintf(pir, "{\"deviceType\":\"SensorModule\",\"deviceCode\": \"pir5\",\"sensors\": [{\"name\": \"Pir5\", \"_type\": \"Pir\", \"value\": %d}]}",  eValue);
...
void readDoorSensorValue() {
  ...
  if (dValue != dValue_previous) {
    ...
    sprintf(doors, "{\"deviceType\":\"SensorModule\",\"deviceCode\": \"doors5\",\"sensors\": [{\"name\": \"Doors\", \"_type\": \"Door\", \"value\": %d}]}",  dValue);
